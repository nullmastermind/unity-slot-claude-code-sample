---
name: "unity-proposal"
description: "Create spec (proposal, design, tasks) for Unity game implementation. Explores and clarifies when needed before creating artifacts."
model: "sonnet"
color: "purple"
---
...
1. **Derive a kebab-case name** from the description (e.g., "slot machine game" → `unity-slot-game`).
...
**Output**: Change name/location, artifacts created, and: "Ready for implementation."
...
- **Annotate verify points in tasks.md** — For the last task of each major group or any high-risk task, append a verify annotation: `← (verify: what to check)`. This tells the verifier WHERE to deep-check and WHAT to look for. Place annotations on tasks that are end-of-flow (everything before must work for this to work) or high-risk (complex logic, integration points, security). Example:
  ...
  1. Project setup
    ...
    1.2 Create scenes ← (verify: all scenes exist in Assets/Scenes/, build settings updated)
  2. Core scripts
    2.1 Create SlotMachine.cs
    2.2 Create ReelController.cs
    2.3 Create PaylineEvaluator.cs ← (verify: all core scripts compile, attached to correct GameObjects)
  3. UI
    3.1 Build MainMenu screen
    3.2 Build Slot HUD
    3.3 Build Paytable overlay ← (verify: all UI screens exist, navigation works, responsive anchoring)
  ```

- **Unity-specific task organization** — when creating tasks from a game blueprint, organize by phase:
  1. Project structure (folders, scenes, packages)
  2. Scene construction (GameObjects, hierarchy, components)
  3. Script writing (in dependency order: data → core → game logic → managers → UI)
  4. UI construction (Canvas, screens, HUD, popups, navigation)
